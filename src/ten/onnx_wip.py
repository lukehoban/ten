...
def make_graph_2() -> onnx.GraphProto:
    ...
    c1 = helper.make_node(
        ...
        [],
        ["c1"],
        value=helper.make_tensor("cc1", onnx.TensorProto.FLOAT, [], [0.7978845608]),
    )
    c2 = helper.make_node(
        "Constant",
        [],
        ["c2"],
        value=helper.make_tensor("cc2", onnx.TensorProto.FLOAT, [], [0.044715]),
    )
    c3 = helper.make_node(
        "Constant",
        [],
        ["c3"],
        value=helper.make_tensor("cc3", onnx.TensorProto.FLOAT, [], [3]),
    )
    c4 = helper.make_node(
        "Constant",
        [],
        ["c4"],
        value=helper.make_tensor("cc4", onnx.TensorProto.FLOAT, [], [1]),
    )
    c5 = helper.make_node(
        "Constant",
        [],
        ["c5"],
        value=helper.make_tensor("cc5", onnx.TensorProto.FLOAT, [], [0.5]),
    )
    ...
    return helper.make_graph(
        nodes=[c1, c2, c3, c4, c5, p1, p2, p3, p4, p5, p6, res],
        name="test",
        inputs=[x],
        outputs=[y],
    )
