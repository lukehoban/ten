...
import unittest
from ten import tenast, compiler, interpreter_numpy as interp
import numpy as np
from typing import Union, Sequence, Any
import baseline
from utils import tensor_type, var, op
...
class InterpreterTestCase(unittest.TestCase):
        ...
        for x in [-1.0, 0.0, 1.0]:
            ...
            ret = i.eval_expr(
                ...
                interp.Env(
                    ...
                    {
                        "x": x,
                        "Tanh_1": interp.Func(tanh),
                        "Tanh_2": interp.Func(tanh),
                        "Tanh_3": interp.Func(tanh),
                    },
                    {},
                ),
            )
        ...
        for x in [-1.0, 0.0, 1.0]:
            ...
            ret = i.eval_expr(
                ...
                interp.Env(
                    ...
                    {
                        "x": x,
                        "Tanh": interp.Func(tanh),
                        "Gelu": interp.Func(self.gelu_decl),
                        "Gelu_2": interp.Func(self.gelu_decl),
                        "Gelu_4": interp.Func(self.gelu_decl),
                        "Gelu_6": interp.Func(self.gelu_decl),
                    },
                    {},
                ),
            )
    ...
    def test_eval_call_linear(self):
        ...
        expected = x @ w + b
        ret = i.eval_call_expr(linear_decl, [w, b], [x], interp.Env(None, {}, {}))
    ...
    def test_eval_call_ffn(self):
        ...
        ret = i.eval_call_expr(
            ...
            interp.Env(
                ...
                {
                    "Gelu_2": interp.Func(self.gelu_decl),
                    "Linear_3": interp.Func(self.linear_decl),
                    "Linear_4": interp.Func(self.linear_decl),
                    **self.built_in_impls,
                },
                {},
            ),
    ...
    def test_eval_call_softmax(self):
        ...
        for arr, expected in [
            ...
            vars: dict[str, interp.Value] = dict(
                {
                    "Exp": interp.Func(exp),
                    "Max": interp.Func(max),
                    "Sum": interp.Func(sum),
