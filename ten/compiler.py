...
"""
A compiler for the Ten language which compiles a tenast into a new tenast with all 
static values resolved and all types inferred and made explicit in the AST.
"""
...
class Compiler:
    ...
    def compile_expr(self, expr: Expr, env: TypeEnv) -> Tuple[Expr, TensorType]:
        """Compiles and type checks the expression.

        Resolves all static values and returns a new expression with all static
        values replaced with constants as well as the type of the expression.
        """
        if isinstance(expr, FloatExpr):
            ...
            # ForExpr is statically compiled into a fixed sequence of LetExpr
            # statements.
